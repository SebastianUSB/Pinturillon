...
function finishedPosition() {
  if (isDrawer && isDrawing) {
      isDrawing = false;
      ctx.beginPath(); // Reinicia el camino para el próximo trazo (falla Toca revisarlo)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Evento: Mensaje de palabra correcta
    socket.on('correct_guess', (data) => {
        const message = `${data.username} ha adivinado la palabra correctamente!`;
        appendMessage(message, 'correct-message');
    });

    // Evento: Finaliza el turno
    socket.on('game_over', (data) => {
        showBootstrapAlert(true, data.message);
    });    


    // Evento: Que usuario es el dibujante
